...
class SlugMapper:
    ...
    def get_slug(self, data_key: str) -> str:
        ...
        if best_match:
            logger.warning(
                f"Using template match '{best_match}' for data key '{data_key}' (generated: '{generated_slug}')"
            )
            return best_match
    ...
    def validate_all_mappings(self, data_keys: List[str]) -> Dict[str, Any]:
        ...
        for data_key in data_keys:
            ...
            required_tokens = {"title": "{{" + f"{slug}_title" + "}}", "chart": "{{" + f"{slug}_chart" + "}}"}

            # Optional tokens that may or may not exist
            optional_tokens = {"paragraph": "{{" + f"{slug}_paragraph" + "}}"}

            all_tokens = {**required_tokens, **optional_tokens}

            # Check if tokens exist in template
            token_matches = {
                token_type: token in self.template_placeholders for token_type, token in all_tokens.items()
            }

            # Only require REQUIRED tokens to match (not paragraph)
            required_tokens_match = all(token_matches[token_type] for token_type in required_tokens.keys())
            ...
            if required_tokens_match:
                ...
                missing_required = [
                    token_type for token_type in required_tokens.keys() if not token_matches[token_type]
                ]
