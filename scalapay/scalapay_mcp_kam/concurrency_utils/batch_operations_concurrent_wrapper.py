...
async def concurrent_batch_text_replace_with_fallback(
    ...
    **kwargs,
    ...
    if enable_concurrent is None:
        enable_concurrent = not (
            EMERGENCY_DISABLE_CONCURRENCY or os.getenv("SCALAPAY_EMERGENCY_MODE", "").lower() in ("true", "1", "yes")
        )
    ...
    try:
        ...
        result = await concurrent_batch_text_replace(
            slides_service, presentation_id, text_map, correlation_id=corr_id, **kwargs
        )
...
async def concurrent_batch_replace_shapes_with_images_and_resize_with_fallback(
    ...
    **kwargs,
    ...
    if not enable_concurrent:
        ...
        return await safe_sequential_batch_image_replace(slides_service, presentation_id, image_map, resize, corr_id)
    ...
    result = await safe_sequential_batch_image_replace(slides_service, presentation_id, image_map, resize, corr_id)
