...
async def enhanced_batch_text_replace_with_styling(
    ...
    **kwargs
    ...
    if enable_styling and results and any(
        ...
        try:
            ...
            logger.info(f"[{corr_id}] Built slide metadata for {len(slide_metadata)} entries: {list(slide_metadata.keys())}")
    ...
    *,
    ...
    if enable_styling and results:
        ...
        text_result = await enhanced_batch_text_replace_with_styling(
            slides_service, presentation_id, text_map,
            results=results, enable_styling=False, correlation_id=f"{corr_id}_text"
        )
        
        image_result = await enhanced_batch_image_replace_with_styling(
            slides_service, presentation_id, image_map,
            results=results, enable_styling=False, correlation_id=f"{corr_id}_image"
        )
