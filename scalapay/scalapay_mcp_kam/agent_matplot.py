...
def _to_dict(x: Any) -> Dict[str, Any]:
    ...
    if isinstance(x, str):
        ...
        try:
            ...
            m = re.search(r"```json\s*(.*?)\s*```", x, flags=re.DOTALL | re.IGNORECASE) \
                or re.search(r"```\s*(.*?)\s*```", x, flags=re.DOTALL)
            if m:
                try:
    ...
    for data_type, entry in results_dict.items():
        if not isinstance(entry, dict):
            results_dict[data_type] = {
                "errors": [f"Invalid entry type: {type(entry).__name__}"],
                "chart_path": None
            }
            continue
        ...
        try:
            ...
            instruction = STRUCTURED_CHART_SCHEMA_PROMPT.format(
                ...
                "General formatting:\n"
                "- Use a clean, modern style matplotlib style sheets)\n"
