...
async def test_upload_single_image_concurrent_success(mock_section):
    ...
    with patch("scalapay.scalapay_mcp_kam.tools_agent_kam_concurrent.upload_png", return_value="test_file_id"), patch(
        "scalapay.scalapay_mcp_kam.tools_agent_kam_concurrent.make_file_public"
    ):
        result = await upload_single_image_concurrent(mock_drive, mock_section, "test_folder")
...
async def test_upload_single_image_concurrent_failure(mock_section):
    ...
    with patch(
        "scalapay.scalapay_mcp_kam.tools_agent_kam_concurrent.upload_png", side_effect=Exception("Upload failed")
    ):
        result = await upload_single_image_concurrent(mock_drive, mock_section, "test_folder")
...
async def test_process_speaker_notes_concurrent():
    ...
    with patch(
        "scalapay.scalapay_mcp_kam.tools_agent_kam_concurrent.add_speaker_notes_to_slides",
        new_callable=AsyncMock,
        return_value={"notes_added": 2},
    ):
        result = await process_speaker_notes_concurrent(
            mock_slides_service, "test_presentation_id", sections, batch_size=1, max_concurrent=2
        )
...
async def test_run_validation_concurrent():
    ...
    with patch(
        "scalapay.scalapay_mcp_kam.utils.slug_validation.debug_slug_mapping", return_value={"success_rate": 0.8}
    ), patch(
        "scalapay.scalapay_mcp_kam.utils.slug_validation.verify_chart_imports", return_value={"success_rate": 0.9}
    ):
        result = await run_validation_concurrent(mock_results, "template_id", "presentation_id", uploaded_files)
...
async def test_fallback_to_sequential_processing(mock_concurrency_config):
    ...
    with patch(
        "scalapay.scalapay_mcp_kam.tools_agent_kam_concurrent.fill_template_for_all_sections_new_enhanced_concurrent",
        side_effect=Exception("Concurrent processing failed"),
    ), patch(
        "scalapay.scalapay_mcp_kam.tools_agent_kam_local.fill_template_for_all_sections_new_enhanced",
        new_callable=AsyncMock,
        return_value={"presentation_id": "test_id", "concurrent_processing_enabled": False},
    ):
