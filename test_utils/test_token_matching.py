...
def test_token_matching():
    ...
    for token in test_tokens:
        ...
        for data_type, metadata in slide_metadata.items():
            ...
            if (any(word in token_clean for word in data_type_clean.split() if len(word) > 3) or
                any(word in data_type_clean for word in token_clean.split() if len(word) > 3)):
        ...
        if best_match_metadata:
            ...
            try:
                ...
                print(f"   🎨 Style: {resize_config.get('scaleX', 120)}×{resize_config.get('scaleY', 120)} at ({resize_config.get('translateX', 130)}, {resize_config.get('translateY', 250)})")
