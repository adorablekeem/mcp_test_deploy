...
class MockSlidesService:
    ...
    def execute(self):
        ...
        if "get" in str(self):
            return {"slides": [{"objectId": "slide_1"}, {"objectId": "slide_2"}, {"objectId": "slide_3"}]}

        # Mock batch update response
        self.requests_received.append({"call": self.call_count, "timestamp": asyncio.get_event_loop().time()})
...
def create_mock_alfred_results() -> Dict[str, Any]:
    ...
    return {
        "monthly sales year over year": {
            ...
            "slides_struct": {
                "structured_data": {"Jan": {"2023": 100, "2024": 150}, "Feb": {"2023": 120, "2024": 180}},
        ...
        "AOV over time": {
            ...
            "slides_struct": {
                "structured_data": {"Jan": 95.5, "Feb": 102.3, "Mar": 98.7, "Apr": 105.2},
...
async def test_enhanced_text_styling():
    ...
    try:
        ...
        for data_type in test_cases:
            ...
            style_config = select_style_config(
                data_type, alfred_results.get(data_type, {}).get("slides_struct", {}).get("paragraph", "")
            )

            print(f"   '{data_type[:30]}...' → {chart_type.value}")
            print(
                f"     Image style: {style_config.image_style.scale_x}x{style_config.image_style.scale_y} at ({style_config.image_style.translate_x}, {style_config.image_style.translate_y})"
            )
