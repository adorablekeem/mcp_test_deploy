...
class ChartGenerator:
    ...
    def create_grouped_bar_chart(
        self, data, title, xlabel="Month", ylabel="Units", output_path="chart.png", figsize=(12, 4), dpi=150
    ):
        """
        ...
        for i, year in enumerate(years):
            ...
            bars = ax.bar(x + i * width - width * (len(years) - 1) / 2, values, width, label=str(year), color=color)
    ...
    def create_line_chart(
        self, data, title, xlabel="Period", ylabel="Value", output_path="line_chart.png", figsize=(12, 4), dpi=150
    ):
        """
        Create a line chart
        ...
        for i, (series_name, series_data) in enumerate(data.items()):
            ...
            ax.plot(x_values, y_values, marker="o", linewidth=2, markersize=6, label=series_name, color=color)
...
def create_revenue_chart(output_path="revenue_chart.png"):
    ...
    revenue_data = {
        "2023": {"Q1": 1.2, "Q2": 1.5, "Q3": 1.8, "Q4": 2.1},
        "2024": {"Q1": 2.3, "Q2": 2.7, "Q3": 3.1, "Q4": 3.5},
    }
    ...
    for i, (year, data) in enumerate(revenue_data.items()):
        ...
        ax.plot(quarters, values, marker="o", linewidth=3, markersize=8, label=year, color=color)
...
def create_custom_chart(data_dict, chart_type="bar", title="Custom Chart", output_path="custom_chart.png"):
    """
    ...
    if chart_type == "bar":
        return generator.create_grouped_bar_chart(data=data_dict, title=title, output_path=output_path)
    elif chart_type == "line":
        return generator.create_line_chart(data=data_dict, title=title, output_path=output_path)
    else:
...
def create_pie_chart(self, data, title, output_path="pie_chart.png", figsize=(6, 6), dpi=150, autopct="%1.1f%%"):
    """
    Create a pie chart.
    ...
    wedges, texts, autotexts = ax.pie(
        values, labels=labels, colors=colors, autopct=autopct, startangle=90, textprops={"fontsize": 10}
    )
...
def create_stacked_bar_chart(
    self, data, title, xlabel="Period", ylabel="Value", output_path="stacked_bar.png", figsize=(12, 4), dpi=150
):
    """
...
def create_line_chart_with_growth(
    self, series_data, title, growth_pct=None, output_path="line_growth.png", figsize=(12, 4), dpi=150
):
    """
    ...
    for i, (series_name, data) in enumerate(series_data.items()):
        x = list(data.keys())
        y = list(data.values())
        ax.plot(x, y, marker="o", linewidth=2, color=self.color_palette[i % len(self.color_palette)], label=series_name)
...
if __name__ == "__main__":
    ...
    generated_files = ["monthly_sales_2023_2025.png", "quarterly_revenue_growth.png", "product_performance.png"]
...
def quick_chart(chart_type, data, title="Quick Chart", output_path="quick_chart.png"):
    ...
    return create_custom_chart(data_dict=data, chart_type=chart_type, title=title, output_path=output_path)


if __name__ == "__main__":
    # Example usage of quick chart
    print("\n🔍 Quick chart example:")
    quick_chart_data = {"Category 1": {2023: 50, 2024: 70}, "Category 2": {2023: 60, 2024: 80}}
